...
function isKeyedItem(
 ...
 return (
 typeof item ==="object" &&
 item!== null &&!Array.isArray(item) &&"key" in (item as object) &&"content" in (item as object)
 );
}
...
const DENSITY_CLASS: Record<BulletedListDensity, string> = {
 tight:"space-y-1"normal:"space-y-2"relaxed:"space-y-3"};

// Dot size scales subtly with text size to keep optical balance.
const DOT_SIZE_CLASS: Record<BulletedListSize, string> = {
 xs:"mt-1 h-1 w-1"sm:"mt-1.5 h-1.5 w-1.5"base:"mt-2 h-2 w-2"lg:"mt-2.5 h-2 w-2"};

// Map tones to semantic tokens declared in src/styles.css.
const TONE_CLASS: Record<BulletedListTone, string> = {
 primary:"bg-primary"accent:"bg-accent"muted:"bg-muted-foreground"foreground:"bg-foreground"destructive:"bg-destructive"success:"bg-emerald-500"// fallback if no semantic success token is defined
};
...
export function BulletedList({
 ...
 className,
 itemClassName,
 size ="sm"density ="normal"tone ="primary"ariaLabel,
 ariaLabelledBy,
}: BulletedListProps) {
 return (
 <ul
 ...
 const key: string | number = isKeyedItem(item)? item.key: typeof item ==="string" || typeof item ==="number"? item: index;
 ...
 <span
 aria-hidden="true"
 className={cn("shrink-0 rounded-full"DOT_SIZE_CLASS[size],
 TONE_CLASS[tone],
 )}
 />
