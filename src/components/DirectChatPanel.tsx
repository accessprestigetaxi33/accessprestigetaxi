...
export function DirectChatPanel({ accountId, authToken, role, onClose, peerName }: Props) {
 const auth = () =>
 role ==="client"? { role:"client" as const, token: authToken??"" }: { role:"chauffeur" as const, token: getDriverToken(), client_account_id: accountId };
 const t = useT();
 const { lang } = useI18n();
 const locale =
 lang ==="fr"?"fr-FR": lang ==="en"?"en-US": lang ==="es"?"es-ES": lang ==="it"?"it-IT": lang ==="pt"?"pt-PT": lang ==="ar"?"ar":"fr-FR";
 ...
 style={
 mine? { background:"linear-gradient(135deg, #C9A84C 0%, #E8C96D 100%)" }: { background:"rgba(255,255,255,0.08)" }
 }
 >
