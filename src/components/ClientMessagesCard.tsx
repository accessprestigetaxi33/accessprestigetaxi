...
import { listDirectMessages, type DirectMessage } from "@/lib/chat.functions";
...
export function ClientMessagesCard({ clientToken }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const list = await listDirectMessages({
  ...
  useEffect(() => {
    const sync = () => {
      ...
      try {
        const keys = Object.keys(localStorage).filter((k) => k.startsWith("chat:offline:client:"));
        let total = 0;
        for (const k of keys) {
