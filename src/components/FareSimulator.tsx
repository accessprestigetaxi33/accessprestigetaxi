...
const LOCAL_COPY = {
 fr: {
 useMyPosition:"📍 Utiliser ma position"geoError:"Impossible d'obtenir votre position"geoUsed:"Position utilisée comme origine"},
 en: {
 useMyPosition:"📍 Use my location"geoError:"Unable to get your location"geoUsed:"Location used as pickup point"},
} as const;
...
function parisHour(date: Date): number {
 ...
 timeZone:"Europe/Paris"hour:"2-digit"hourCycle:"h23"}).formatToParts(date);
...
export function FareSimulator() {
 ...
 const periodLabel = isDay? `${t("sim.period_day")} ${formatEUR(RATE_DAY)} / km`: `${t("sim.period_night")} ${formatEUR(RATE_NIGHT)} / km`;
