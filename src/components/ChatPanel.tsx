...
export function ChatPanel({ reservationId, role, onClose, peerName, clientToken }: Props) {
 ...
 type:"broadcast"event:"typing"payload: { role },
 });
 }
 ...
 const lines = [
 header.join(""),...rows.map((m) =>
 [
 new Date(m.created_at).toISOString(),
 m.sender,
 escape(m.content),
 m.read_by_client?"1":"0"m.read_by_chauffeur?"1":"0"].join(""),
 ),
 ];
 ...
 height:"100dvh"maxHeight:"100dvh"borderTop: isDriver?"1px solid #E8DFCB":"1px solid rgba(255,255,255,0.1)"}}
 >
 {/* Header */}
 <div
 ...
 background: isDriver?"linear-gradient(180deg, #F5EEDC 0%, #FBF7EE 100%)":"linear-gradient(180deg, rgba(201,168,76,0.12) 0%, transparent 100%)"}}
 >
 {isDriver && (
 <button
 onClick={onClose}
 ...
 style={
 mine? { background:"linear-gradient(135deg, #C9A84C 0%, #E8C96D 100%)" }: isDriver? { background:"#ffffff"border:"1px solid #E8DFCB" }: { background:"rgba(255,255,255,0.08)" }
 }
 >
 ...
 style={{
 borderTop: isDriver?"1px solid #E8DFCB":"1px solid rgba(255,255,255,0.1)"background: isDriver?"#F5EEDC":"rgba(0,0,0,0.3)"paddingBottom:"calc(0.75rem + env(safe-area-inset-bottom, 0px))"}}
 >
 <textarea
 value={input}
 onChange={(e) => {
 ...
 rows={1}
 className={
 isDriver?"max-h-32 flex-1 resize-none rounded-xl border border-black/10 bg-white px-3 py-2.5 text-sm text-black placeholder-black/40 outline-none focus:border-[#C9A84C]":"max-h-32 flex-1 resize-none rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white placeholder-white/40 outline-none focus:border-[#E8C96D]"
 }
 />
 <button
 type="submit"
