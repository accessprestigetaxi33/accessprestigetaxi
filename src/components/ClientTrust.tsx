...
const COPY = {
 fr: {
 eyebrow:"Avis & réassurance"title:"La confiance de nos clients"lead:"Ponctualité, sécurité et discrétion: ce que nos passagers retiennent de Patricia et Alain."reviews: (n: number) => `${n} avis vérifié${n > 1?"s":""}`,
 noReviews:"Soyez le premier à partager votre expérience — les avis publiés proviennent de courses réellement effectuées."outOf:"sur 5"trust: [
 ...
 { icon: Sparkles, t:"Confort 100 % électrique"d:"BMW iX1 électrique / van Mercedes 7 places silencieuses, eau, chargeurs et Wi-Fi à bord." },
 ],
 verified:"Avis vérifié"safety:"Sécurité, assurance & garanties"moderation:"Chaque avis est relu par Patricia et Alain avant publication."},
 en: {
 eyebrow:"Reviews & reassurance"title:"Trusted by our passengers"lead:"Punctuality, safety and discretion: what riders remember about Patricia and Alain."reviews: (n: number) => `${n} verified review${n > 1?"s":""}`,
 noReviews:"Be the first to share your experience — published reviews come from real completed rides."outOf:"out of 5"trust: [
 ...
 { icon: Sparkles, t:"100% electric comfort"d:"Silent BMW iX1 électrique / van Mercedes 7 places with water, chargers and on-board Wi-Fi." },
 ],
 verified:"Verified review"safety:"Safety, insurance & guarantees"moderation:"Every review is checked by Patricia and Alain before publication."},
} as const;
...
export function ClientTrust() {
 ...
 useEffect(() => {
 let active = true;
 supabase.from("avis").select("id,author_name,note,commentaire,created_at").eq("status""approved").order("created_at"{ ascending: false }).limit(6).then(({ data }) => {
 ...
 const jsonLd =
 count > 0? {"@context":"https://schema.org""@type":"TaxiService"name:"Access Prestige Taxi"areaServed: ["Charente-Maritime"],
 aggregateRating: {"@type":"AggregateRating"ratingValue: average.toFixed(1),
 reviewCount: count,
 bestRating:"5"worstRating:"1"},
 review: withText.map((r) => ({"@type":"Review"author: {"@type":"Person"name: r.author_name?.trim() ||"Client" },
