...
const COPY = {
 fr: {
 eyebrow:"Le guide Access Prestige Taxi"title:"Charente-Maritime: où manger, dormir, marcher et s'émerveiller"lead:"Restaurants de caractère, hôtels étoilés, randonnées côtières et sites chargés d'histoire — sélectionnés par nos deux chauffeurs, et desservis en BMW iX1 100 % électrique."all:"Tout"filterCat:"Catégorie"filterDept:"Département"read:"Lire l'article"count: (n: number) => `${n} adresse${n > 1?"s":""}`,
 ctaTitle:"On vous y emmène"ctaText:"Trajet à la demande depuis Bordeaux, la Gironde et toute la Nouvelle-Aquitaine."ctaBtn:"Réserver ma course"},
 en: {
 eyebrow:"The Access Prestige Taxi guide"title:"Charente-Maritime: where to eat, sleep, walk and wonder"lead:"Characterful restaurants, star-rated hotels, coastal hikes and history-rich sites — picked by our two drivers and served in a fully electric BMW iX1."all:"All"filterCat:"Category"filterDept:"Area"read:"Read the article"count: (n: number) => `${n} place${n > 1?"s":""}`,
 ctaTitle:"We'll take you there"ctaText:"On-demand rides from Bordeaux, the Gironde and across Nouvelle-Aquitaine."ctaBtn:"Book a ride"},
} as const;

const BLOG_TITLE ="Guide Charente-Maritime — Restaurants, hôtels, randos";
const BLOG_DESC ="Guide complet de la Charente-Maritime: restaurants étoilés, hôtels classés par étoiles, randonnées et lieux à visiter avec leur histoire. Transport en taxi 100 % électrique.";
const BLOG_OG_DESC ="Restaurants, hôtels étoilés, randonnées et sites historiques de Charente-Maritime, avec photos et conseils.";
...
export const Route = createFileRoute("/blog/")({
 ...
 {"@type":"CollectionPage"name: BLOG_TITLE,
...
function BlogIndex() {
 ...
 {isEn? GUIDE_CATEGORIES.find((g) => g.key === e.category)!.en: GUIDE_CATEGORIES.find((g) => g.key === e.category)!.fr}
 </span>
 </div>
 </Link>
...
function Chip({
 ...
 onClick={onClick}
 aria-pressed={active}
 className={["shrink-0 whitespace-nowrap rounded-full border px-3.5 text-xs font-semibold transition"subtle?"py-1.5":"py-2"active?"border-primary bg-primary text-primary-foreground":"border-border bg-card text-foreground/80 hover:border-primary/60"].join("")}
 >
 {label}
 </button>
 );
