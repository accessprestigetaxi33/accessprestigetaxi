 ...
 * { -webkit-tap-highlight-color: transparent; box-sizing: border-box; touch-action: manipulation; }
 ...
 input, textarea, select { font-size: 16px; }.client-login-root {
...
export const Route = createFileRoute("/client/login")({
 ...
 {
 name:"description"content:"Connectez-vous à votre espace client Access Prestige Taxi pour suivre vos courses et gérer vos réservations."},
...
function ClientLoginPage() {
 ...
 try {
 const session =
 mode ==="login"? await clientLogin({ data: { email: email.trim(), password } }): await clientRegister({ data: { email: email.trim(), password, name: name.trim(), phone: phone.trim() } });
...
function Field({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
 ...
 style={{
 height: 50,
 background:"rgba(0,0,0,0.25)"borderColor:"rgba(255,255,255,0.08)"}}
 >
