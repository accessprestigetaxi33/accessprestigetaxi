...
const DEFAULTS: Form = {
 mode:"url"url:"https://accessprestigetaxi.lovable.app/carte"name:"Josè"phone:"0650260015"email:"taxi.city033@gmail.com"site:"https://accessprestigetaxi.lovable.app"org:"Access Prestige Taxi"};
...
function buildVCard(f: Form): string {
 ...
 return ["BEGIN:VCARD""VERSION:3.0"`FN:${f.name}`,
 `N:${f.name};;;;`,
 `ORG:${f.org}`,
 tel? `TEL;TYPE=CELL:${tel}`:""f.email? `EMAIL:${f.email}`:""f.site? `URL:${f.site}`:"""END:VCARD"].filter(Boolean).join("\n");
}
...
async function renderQr(
 ...
 width: size,
 margin: 0,
 errorCorrectionLevel:"H"color: { dark:"#000000"light:"#FFFFFF" },
 });
...
function QrGeneratorPage() {
 ...
 const pdf = new jsPDF({
 orientation:"portrait"unit:"mm"format: [55, 55],
 compress: true,
 });
 ...
 <main
 style={{
 minHeight:"100dvh"background:"linear-gradient(180deg,#0a0a0a 0%,#111827 100%)"color:"#fff"padding:"40px 20px"}}
 >
 ...
 border:"none"background: form.mode === m?"#E8C96D":"transparent"color: form.mode === m?"#000":"#fff"fontWeight: 600,
 fontSize: 13,
 cursor:"pointer"}}
 >
 ...
 {form.mode ==="url"?"Le QR pointe vers /carte: boutons Appeler, WhatsApp, SMS, Email, Réserver, Ajouter contact — tout est cliquable.":"Le QR contient une vCard: le téléphone propose « Ajouter aux contacts »."}
 </div>
 ...
 style={{
 background:"#0f172a"border: `1px solid ${errors[key]?"#ef4444":"rgba(255,255,255,0.15)"}`,
 borderRadius: 8,
 padding:"10px 12px"color:"#fff"fontSize: 14,
 }}
 />
 ...
 <div
 style={{
 position:"relative"width: PREVIEW_SIZE_PX,
 ...
 border:"2px dashed #C9A84C"borderRadius: 4,
 pointerEvents:"none"}}
 />
 <canvas
 ref={previewRef}
 ...
 left:"50%"transform:"translate(-50%,-100%)"fontSize: 11,
 ...
 transform:"translate(100%,-50%)"fontSize: 11,
 color:"#C9A84C"fontWeight: 600,
 letterSpacing:"0.1em"}}
 >
 55 mm
...
function slug(s: string) {
 return s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g"").replace(/[^a-z0-9]+/g"").replace(/^-+|-+$/g"") ||"qr";
}
...
function btnGhost(disabled: boolean): React.CSSProperties {
 ...
 border:"1px solid rgba(255,255,255,0.2)"background:"transparent"color:"#fff"fontWeight: 600,
...
function btnGold(disabled: boolean): React.CSSProperties {
 return {
 padding:"12px 16px"borderRadius: 10,
 border:"none"background:"linear-gradient(135deg,#C9A84C,#E8C96D)"color:"#000"fontWeight: 700,
 cursor: disabled?"not-allowed":"pointer"opacity: disabled? 0.5: 1,
 };
}
