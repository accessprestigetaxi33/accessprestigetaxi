...
function ClientTrajets() {
 ...
 const channel = supabase.channel("trajets-realtime").on("postgres_changes"{ event:"UPDATE"schema:"public"table:"reservations"filter: `id=in.(${ids.join("")})` },
 () => refresh(),
 ).subscribe();
 return () => {
 ...
 <li
 key={r.id}
 className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur sm:p-5"
 >
