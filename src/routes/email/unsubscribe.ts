...
export const Route = createFileRoute("/email/unsubscribe")({
 ...
 const { data: updated, error: updateError } = await supabase.from('email_unsubscribe_tokens').update({ used_at: new Date().toISOString() }).eq('token'token).is('used_at'null).select().maybeSingle()
