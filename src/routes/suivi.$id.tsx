...
const UI = {
 fr: {
 you:"Vous"tbd:"À définir"},
 en: {
 you:"You"tbd:"To be determined"},
} as const;
 ...
 * { box-sizing: border-box; }
 ...
 input, textarea { -webkit-tap-highlight-color: transparent; }.suivi-root { overscroll-behavior-y: contain; }
...
> = {
 pending: {
 label:"suivi.status.pending"color:"#92400e"bgGradient:"linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)"borderColor:"rgba(217, 119, 6, 0.2)"icon:"⏳"},
 accepted: {
 label:"suivi.status.accepted"color:"#15803d"bgGradient:"linear-gradient(135deg, #dcfce7 0%, #bbf7d0 100%)"borderColor:"rgba(34, 197, 94, 0.2)"icon:"✨"},
 en_route: {
 label:"suivi.status.en_route"color:"#1d4ed8"bgGradient:"linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%)"borderColor:"rgba(29, 78, 216, 0.2)"icon:"🚕"},
 arrived: {
 label:"suivi.status.arrived"color:"#7c3aed"bgGradient:"linear-gradient(135deg, #ede9fe 0%, #ddd6fe 100%)"borderColor:"rgba(124, 58, 237, 0.2)"icon:"📍"},
 completed: {
 label:"suivi.status.completed"color:"#475569"bgGradient:"linear-gradient(135deg, #f1f5f9 0%, #e2e8f0 100%)"borderColor:"rgba(71, 85, 105, 0.2)"icon:"✓"},
 cancelled: {
 label:"suivi.status.cancelled"color:"#991b1b"bgGradient:"linear-gradient(135deg, #fee2e2 0%, #fecaca 100%)"borderColor:"rgba(185, 28, 28, 0.2)"icon:"✕"},
};
...
function PremiumTimeline({ status }: { status: string }) {
 ...
 <div
 style={{
 display:"flex"flexDirection:"column"alignItems:"center"gap:"5px"flex:"0 0 auto"width:"62px"}}
 >
 <div
 className={`suivi-premium ${isActive && status ==="arrived"?"suivi-pulse-active":""}`}
 style={{
 width:"30px"height:"30px"borderRadius:"50%"background: isDone? config.bgGradient:"#e2e8f0"border: `2px solid ${isDone? config.borderColor:"transparent"}`,
 ...
 color: isDone? config.color:"#94a3b8"textAlign:"center"lineHeight: 1.25,
 width:"100%"whiteSpace:"normal"wordBreak:"normal"overflowWrap:"normal"hyphens:"none"letterSpacing:"0.1px"}}
 >
 {stepLabels[s]}
 </div>
 </div>
 ...
 <div
 style={{
 height:"2px"background: isDone? `linear-gradient(90deg, ${config.color}40, ${config.color}70)`:"#e2e8f0"borderRadius:"1px"transition:"all 0.4s ease"}}
 />
 </div>
 )}
 </div>
...
function ChatSection({ suiviKey, reservationId, t }: { suiviKey: string; reservationId: string; t: (k: string) => string }) {
 ...
 color:"#0f172a"marginBottom:"12px"display:"flex"gap:"8px"alignItems:"center"}}
 >
...
function AnonChat({
 ...
 const ch = (supabase as any).channel(`chat_suivi_${reservationId}`).on("postgres_changes"{
 ...
 type:"broadcast"event:"new_client_message"payload: { at: Date.now() },
 });
 setText("");
 } catch (e: any) {
 ...
 style={{
 flex: 1,
 overflowY:"auto"display:"flex"flexDirection:"column"gap:"8px"paddingRight:"4px"}}
 >
 {messages.length === 0 && (
 ...
 <div
 style={{
 background: mine?"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)":"#f1f5f9"color: mine?"#fff":"#0f172a"padding:"8px 12px"borderRadius:"10px"fontSize:"13px"wordBreak:"break-word"}}
 >
 {msg.content}
 </div>
 ...
 style={{
 flex: 1,
 padding:"12px 12px"borderRadius:"8px"border:"1px solid #e2e8f0"fontSize:"16px"fontFamily:"inherit"transition:"all 0.3s"color:"#0f172a"background:"#ffffff"minHeight:"44px"boxSizing:"border-box"}}
 />
 <button
 onClick={send}
 disabled={sending ||!text.trim()}
 style={{
 padding:"10px 16px"minWidth:"44px"minHeight:"44px"background: sending ||!text.trim()?"#cbd5e1":"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)"color:"#fff"border:"none"borderRadius:"8px"cursor: sending ||!text.trim()?"not-allowed":"pointer"display:"flex"alignItems:"center"justifyContent:"center"gap:"6px"transition:"all 0.3s"}}
 >
...
function generateICS(reservation: any, t: (k: string) => string): string {
 ...
 const lines = ["BEGIN:VCALENDAR""VERSION:2.0""PRODID:-//Access Prestige Taxi//FR""BEGIN:VEVENT"`UID:tcb-${reservation.id}@accessprestigetaxi.lovable.app`,
 ...
 `LOCATION:${reservation.depart}`"END:VEVENT""END:VCALENDAR"].join("\r\n");
...
function InvoiceBlock({ reservation, locale, t }: { reservation: any; locale: string; t: (k: string) => string }) {
 ...
 const dateStr = reservation.pickup_datetime? new Date(reservation.pickup_datetime).toLocaleString(intlLocale, {
 dateStyle:"long"timeStyle:"short"timeZone:"Europe/Paris"}): new Date().toLocaleDateString(intlLocale);
 const prix = reservation.prix_estime? new Intl.NumberFormat(intlLocale, { style:"currency"currency:"EUR" }).format(reservation.prix_estime):"—";
...
<style>
 ...
 body { font-family:'Helvetica Neue'Arial, sans-serif; color: #1a1a1a; max-width: 700px; margin: 40px auto; padding: 0 24px; }.header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 3px solid #1d4ed8; padding-bottom: 20px; margin-bottom: 28px; }.brand { font-size: 22px; font-weight: 900; color: #1d4ed8; letter-spacing: -0.5px; }.brand small { display: block; font-size: 12px; font-weight: 400; color: #64748b; margin-top: 2px; }.meta { text-align: right; font-size: 12px; color: #64748b; }.meta strong { display: block; font-size: 16px; color: #1a1a1a; font-weight: 700; }
 h2 { font-size: 13px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 12px; }.row { display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #f1f5f9; font-size: 14px; }.row:last-child { border: none; }.row.label { color: #475569; }.row.value { font-weight: 600; }.total-box { margin-top: 24px; background: #f8fafc; border-radius: 10px; padding: 16px 20px; display: flex; justify-content: space-between; align-items: center; }.total-box.label { font-size: 14px; color: #475569; }.total-box.amount { font-size: 26px; font-weight: 900; color: #1d4ed8; }.footer { margin-top: 40px; padding-top: 16px; border-top: 1px solid #e2e8f0; font-size: 11px; color: #94a3b8; text-align: center; }.btn { display: inline-block; margin: 20px 8px 0; padding: 10px 24px; background: #1d4ed8; color: #fff; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; }
</style></head><body>
<div class="header">
...
<div class="footer">${labelFooterLegal}<br/>${labelFooterThanks}</div>
 ...
 color:"#1d4ed8"textTransform:"uppercase"letterSpacing:"0.5px"marginBottom:"14px"display:"flex"alignItems:"center"gap:"6px"}}
 >
 ...
 <div
 style={{
 display:"flex"justifyContent:"space-between"fontSize:"13px"paddingBottom:"8px"borderBottom:"1px solid #f1f5f9"}}
 >
 ...
 <div
 style={{
 fontSize:"11px"color:"#94a3b8"textAlign:"right"marginTop:"2px"}}
 >
 ...
 onClick={handlePrint}
 style={{
 display:"flex"alignItems:"center"justifyContent:"center"gap:"6px"padding:"12px 10px"background:"rgba(255,255,255,0.08)"color:"#94a3b8"border:"1px solid rgba(148,163,184,0.2)"borderRadius:"10px"fontSize:"13px"fontWeight: 600,
 cursor:"pointer"}}
 >
 ...
 style={{
 display:"flex"alignItems:"center"justifyContent:"center"gap:"6px"padding:"12px 10px"background:"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)"color:"#fff"border:"none"borderRadius:"10px"fontSize:"13px"fontWeight: 600,
 ...
 style={{
 gridColumn:"1 / -1"display:"flex"alignItems:"center"justifyContent:"center"gap:"6px"padding:"12px 10px"background: emailSent?"linear-gradient(135deg, #16a34a 0%, #15803d 100%)": emailSending?"#cbd5e1":"linear-gradient(135deg, #0ea5e9 0%, #0284c7 100%)"color:"#fff"border:"none"borderRadius:"10px"fontSize:"13px"fontWeight: 600,
 cursor: emailSending || emailSent?"not-allowed":"pointer"boxShadow: emailSent?"0 4px 12px rgba(22,163,74,0.3)":"0 4px 12px rgba(14,165,233,0.3)"}}
 >
 {emailSending? (
...
function RecurringModal({ reservation, onClose }: { reservation: any; onClose: () => void }) {
 ...
 mode_paiement: reservation.mode_paiement??"cb"client_name: reservation.client_name,
 frequency: freq,
 ...
 background:"rgba(0,0,0,0.55)"display:"flex"alignItems:"flex-end"justifyContent:"center"zIndex: 9999,
 backdropFilter:"blur(4px)"}}
 >
 <div
 onClick={(e) => e.stopPropagation()}
 style={{
 background:"#fff"borderRadius:"20px 20px 0 0"padding:"24px 20px calc(24px + env(safe-area-inset-bottom, 0px))"width:"100%"maxWidth: 480,
 maxHeight:"90dvh"overflowY:"auto"boxShadow:"0 -8px 40px rgba(0,0,0,0.18)"boxSizing:"border-box"}}
 >
 ...
 fontSize: 11,
 fontWeight: 700,
 border:"2px solid"borderColor: freq === f?"#1d4ed8":"#e2e8f0"background: freq === f?"#eff6ff":"#fff"color: freq === f?"#1d4ed8":"#64748b"cursor:"pointer"}}
 >
 {freqLabel[f]}
 </button>
 ))}
 ...
 fontSize: 12,
 fontWeight: 700,
 border:"2px solid"borderColor: dayOfWeek === i?"#1d4ed8":"#e2e8f0"background: dayOfWeek === i?"#eff6ff":"#fff"color: dayOfWeek === i?"#1d4ed8":"#64748b"cursor:"pointer"}}
 >
 {label.slice(0, 3)}
 </button>
 ))}
 ...
 color:"#0f172a"fontFamily:"inherit"boxSizing:"border-box" as const,
 }}
 />
 </div>
 ...
 style={{
 width:"100%"padding:"15px 16px"background: saved?"linear-gradient(135deg, #16a34a 0%, #15803d 100%)":"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)"color:"#fff"border:"none"borderRadius: 12,
 fontSize: 15,
 fontWeight: 800,
 cursor: saving || saved?"not-allowed":"pointer"boxShadow:"0 4px 16px rgba(29,78,216,0.35)"display:"flex"alignItems:"center"justifyContent:"center"gap: 8,
 }}
 >
...
function ReviewBlock({ reservationId, authorName, t }: { reservationId: string; authorName?: string | null; t: (k: string) => string }) {
 ...
 style={{
 fontSize:"12px"fontWeight: 700,
 color:"#f59e0b"textTransform:"uppercase"letterSpacing:"0.5px"marginBottom:"14px"display:"flex"alignItems:"center"gap:"6px"}}
 >
 ...
 style={{
 background:"none"border:"none"cursor:"pointer"padding:"6px"minWidth:"44px"minHeight:"44px"display:"flex"alignItems:"center"justifyContent:"center"transition:"transform 0.15s"transform: (hover || rating) >= star?"scale(1.2)":"scale(1)"}}
 >
 <Star
 size={32}
 ...
 rows={3}
 style={{
 width:"100%"padding:"10px 12px"borderRadius:"10px"border:"1px solid #e2e8f0"fontSize:"16px"fontFamily:"inherit"resize:"vertical"marginBottom:"12px"boxSizing:"border-box"background:"#f8fafc"color:"#0f172a"}}
 />
 ...
 style={{
 width:"100%"padding:"12px 16px"background: rating === 0 || submitting?"#e2e8f0":"linear-gradient(135deg, #f59e0b 0%, #d97706 100%)"color: rating === 0 || submitting?"#94a3b8":"#fff"border:"none"borderRadius:"10px"fontSize:"13px"fontWeight: 700,
 cursor: rating === 0 || submitting?"not-allowed":"pointer"display:"flex"alignItems:"center"justifyContent:"center"gap:"6px"transition:"all 0.3s"}}
 >
...
function ShareTrajetButton({ reservation }: { reservation: any }) {
 ...
 style={{
 background:"linear-gradient(135deg, #0ea5e9 0%, #0284c7 100%)"border:"none"borderRadius:"8px"padding:"8px 12px"minHeight:"36px"cursor:"pointer"display:"flex"alignItems:"center"gap:"4px"fontSize:"11px"fontWeight: 700,
 color:"#fff"boxShadow:"0 2px 8px rgba(14,165,233,0.35)"}}
 >
 <Share2 size={13} />
 {t("suivi.share")}
 </button>
 ...
 left: 0,
 right: 0,
 background:"#fff"borderRadius:"20px 20px 0 0"padding:"20px 20px calc(24px + env(safe-area-inset-bottom, 0px))"zIndex: 9999,
 boxShadow:"0 -8px 40px rgba(0,0,0,0.18)"maxWidth: 480,
 maxHeight:"90dvh"overflowY:"auto"margin:"0 auto"boxSizing:"border-box"}}
 >
 ...
 padding:"13px 16px"background:"linear-gradient(135deg, #16a34a 0%, #15803d 100%)"color:"#fff"borderRadius: 12,
 ...
 style={{
 display:"flex"alignItems:"center"gap: 10,
 padding:"13px 16px"background:"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)"color:"#fff"borderRadius: 12,
 textDecoration:"none"fontWeight: 700,
 fontSize: 14,
 ...
 padding:"13px 16px"background:"rgba(248,250,252,1)"color:"#334155"border:"1px solid #e2e8f0"borderRadius: 12,
 fontWeight: 700,
 fontSize: 14,
 cursor:"pointer"}}
 >
...
function SuiviPage() {
 ...
 const ch = (supabase as any).channel(`suivi:${resolvedId}`, { config: { broadcast: { self: false } } }).on("broadcast"{ event:"update" }, () => {
 ...
 event_type:"tracking_opened"source: src,
 ...
 const channel = taxiSupabase.channel(`reservations:id=eq.${resolvedId}_${Date.now()}`).on("postgres_changes"{ event:"UPDATE"schema:"public"table:"reservations"filter: `id=eq.${resolvedId}` },
 (payload: any) => {
 try {
 lastUpdateRef.current = Date.now();
 setStaleMinutes(0);
 ...
 <div
 style={{
 minHeight:"100dvh"display:"flex"alignItems:"center"justifyContent:"center"background:"linear-gradient(135deg, #0f172a 0%, #1e293b 100%)"}}
 >
 ...
 <div
 style={{
 minHeight:"100dvh"display:"flex"alignItems:"center"justifyContent:"center"background:"linear-gradient(135deg, #0f172a 0%, #1e293b 100%)"padding:"20px"}}
 >
 <style>{PREMIUM_CSS}</style>
 ...
 to="/"
 style={{
 color:"#1d4ed8"textDecoration:"none"fontSize:"14px"marginTop:"20px"display:"inline-block"}}
 >
 ...
 href="/reserver"
 style={{
 display:"inline-flex"alignItems:"center"gap:"8px"padding:"12px 24px"background:"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)"color:"#fff"borderRadius:"10px"textDecoration:"none"fontWeight: 700,
 fontSize:"14px"boxShadow:"0 4px 12px rgba(29,78,216,0.3)"}}
 >
 {t("suivi.expired_cta")}
 </a>
 </div>
 ...
 style={{
 background:"linear-gradient(135deg, #0f172a 0%, #1e293b 100%)"minHeight:"100dvh"padding:"16px"paddingTop:"calc(16px + env(safe-area-inset-top, 0px))"paddingBottom:"calc(32px + env(safe-area-inset-bottom, 0px))"fontFamily:"apple-system, BlinkMacSystemFont'Segoe UI'sans-serif"maxWidth:"100vw"overflowX:"hidden"boxSizing:"border-box"}}
 >
 ...
 style={{
 display:"inline-flex"alignItems:"center"gap:"6px"padding:"8px 14px"background:"rgba(255,255,255,0.08)"border:"1px solid rgba(255,255,255,0.15)"borderRadius:"10px"color:"#94a3b8"fontSize:"13px"fontWeight: 600,
 textDecoration:"none"}}
 >
 ...
 <div
 style={{
 marginBottom:"12px"padding:"10px 14px"background:"rgba(239,68,68,0.15)"border:"1px solid rgba(239,68,68,0.3)"borderRadius:"10px"display:"flex"alignItems:"center"gap:"8px"}}
 >
 ...
 <div
 style={{
 marginBottom:"12px"padding:"10px 14px"background:"rgba(245,158,11,0.15)"border:"1px solid rgba(245,158,11,0.3)"borderRadius:"10px"display:"flex"alignItems:"center"justifyContent:"space-between"gap:"8px"}}
 >
 ...
 color:"#f59e0b"background:"none"border:"none"cursor:"pointer"textDecoration:"underline"padding: 0,
 }}
 >
 ...
 <div
 style={{
 background:"linear-gradient(135deg, rgba(255,255,255,0.95) 0%, rgba(248,250,252,0.9) 100%)"backdropFilter:"blur(20px)"borderRadius:"16px"border:"1px solid rgba(148, 163, 184, 0.2)"padding:"20px"boxShadow:"0 8px 32px rgba(15, 23, 42, 0.1)"}}
 >
 {/* Heure de prise en charge — très visible */}
 {reservation.pickup_datetime &&!isCompleted && (
 <div
 style={{
 background:"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)"borderRadius:"10px"padding:"10px 14px"marginBottom:"14px"display:"flex"alignItems:"center"gap:"10px"}}
 >
 ...
 color:"rgba(255,255,255,0.7)"textTransform:"uppercase"letterSpacing:"0.5px"}}
 >
 ...
 style={{
 marginLeft:"auto"display:"flex"alignItems:"center"gap:"5px"padding:"6px 10px"background:"rgba(255,255,255,0.15)"borderRadius:"8px"color:"#fff"textDecoration:"none"fontSize:"11px"fontWeight: 600,
 flexShrink: 0,
 }}
 >
 ...
 <div
 style={{
 display:"flex"alignItems:"flex-start"justifyContent:"space-between"marginBottom:"16px"}}
 >
 <div>
 ...
 <div
 style={{
 marginTop:"8px"display:"flex"alignItems:"center"gap:"6px"padding:"5px 10px"background:"linear-gradient(135deg, #dcfce7 0%, #bbf7d0 100%)"borderRadius:"20px"border:"1px solid rgba(34,197,94,0.2)"width:"fit-content"}}
 >
 ...
 style={{
 background:"rgba(148,163,184,0.1)"border:"1px solid rgba(148,163,184,0.2)"borderRadius:"8px"padding:"8px 10px"minHeight:"36px"cursor:"pointer"display:"flex"alignItems:"center"gap:"4px"fontSize:"11px"fontWeight: 600,
 color:"#64748b"}}
 >
 <Share2 size={13} />
 {t("suivi.share")}
 ...
 style={{
 marginTop:"12px"background:"linear-gradient(135deg, #1e293b 0%, #0f172a 100%)"borderRadius:"12px"border:"1px solid rgba(255,255,255,0.08)"overflow:"hidden"}}
 >
 {/* Photo du véhicule */}
 <div
 style={{
 position:"relative"width:"100%"height:"clamp(140px, 42vw, 190px)"overflow:"hidden"}}
 >
 <img
 ...
 const parent = (e.target as HTMLImageElement).closest(".vehicle-photo-block") as HTMLElement | null;
 if (parent) parent.style.display ="none";
 }}
 style={{
 width:"100%"height:"100%"//"contain" au lieu de"cover": la hauteur du cadre était fixe (140px)
 ...
 left: 0,
 right: 0,
 height:"60px"background:"linear-gradient(to top, #0f172a, transparent)"}}
 />
 {/* Badge Taxi flottant */}
 <div
 style={{
 position:"absolute"top:"10px"right:"10px"background:"rgba(0,0,0,0.6)"backdropFilter:"blur(8px)"borderRadius:"20px"padding:"4px 10px"fontSize:"11px"fontWeight: 700,
 ...
 <div
 style={{
 padding:"5px 10px"background:"#fff"borderRadius:"6px"border:"2px solid #003189"display:"flex"flexDirection:"column"alignItems:"center"flexShrink: 0,
 }}
 >
 <div
 style={{
 fontSize:"13px"fontWeight: 900,
 color:"#1a1a1a"letterSpacing:"1px"lineHeight: 1.2,
 }}
 >
 {VEHICLE_PLATE}
 </div>
 ...
 color:"#94a3b8"textTransform:"uppercase"letterSpacing:"0.5px"display:"flex"alignItems:"center"gap:"4px"}}
 >
 ...
 <div
 style={{
 height:"1px"background:"linear-gradient(90deg, transparent 0%, #e2e8f0 50%, transparent 100%)"}}
 />
 <div style={{ display:"flex"gap:"12px"alignItems:"flex-start" }}>
 ...
 className="suivi-premium suivi-card"
 style={{
 padding:"14px"textAlign:"center"background:"linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)"}}
 >
 ...
 style={{
 fontSize:"12px"fontWeight: 700,
 color:"#1d4ed8"textTransform:"uppercase"letterSpacing:"0.5px"marginBottom:"12px"}}
 >
 ...
 href={`tel:${josePhone}`}
 style={{
 display:"flex"alignItems:"center"gap:"10px"padding:"11px 14px"background:"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)"color:"#fff"borderRadius:"10px"textDecoration:"none"fontWeight: 600,
 fontSize:"13px"border:"none"transition:"all 0.3s"cursor:"pointer"boxShadow:"0 4px 12px rgba(29, 78, 216, 0.3)"}}
 >
 <Phone size={16} />
 ...
 rel="noopener noreferrer"
 style={{
 display:"flex"alignItems:"center"gap:"10px"padding:"11px 14px"background:"linear-gradient(135deg, #16a34a 0%, #15803d 100%)"color:"#fff"borderRadius:"10px"textDecoration:"none"fontWeight: 600,
 fontSize:"13px"border:"none"transition:"all 0.3s"cursor:"pointer"boxShadow:"0 4px 12px rgba(22, 163, 74, 0.3)"}}
 >
 <MessageCircle size={16} />
 ...
 style={{
 display:"flex"alignItems:"center"justifyContent:"center"gap:"8px"padding:"13px 16px"background:"linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)"color:"#fff"borderRadius:"10px"textDecoration:"none"fontWeight: 700,
 fontSize:"14px"boxShadow:"0 4px 12px rgba(29, 78, 216, 0.3)"}}
 >
 ...
 style={{
 display:"flex"alignItems:"center"justifyContent:"center"gap:"8px"padding:"13px 16px"background:"linear-gradient(135deg, #7c3aed 0%, #6d28d9 100%)"color:"#fff"border:"none"borderRadius:"10px"fontWeight: 700,
 fontSize:"14px"cursor:"pointer"boxShadow:"0 4px 12px rgba(124, 58, 237, 0.3)"}}
 >
 ...
 to="/"
 style={{
 display:"flex"alignItems:"center"justifyContent:"center"padding:"11px 16px"background:"rgba(255,255,255,0.08)"color:"#94a3b8"borderRadius:"10px"textDecoration:"none"fontWeight: 600,
 fontSize:"13px"border:"1px solid rgba(148,163,184,0.15)"}}
 >
 {t("suivi.back_home")}
 </Link>
 </div>
 ...
 disabled={refreshing}
 style={{
 width:"100%"padding:"13px 16px"background: refreshing?"rgba(255,255,255,0.05)":"rgba(255,255,255,0.08)"color: refreshing?"#64748b":"#94a3b8"border:"1px solid rgba(148,163,184,0.15)"borderRadius:"12px"fontWeight: 600,
 fontSize:"13px"cursor: refreshing?"not-allowed":"pointer"display:"flex"alignItems:"center"justifyContent:"center"gap:"8px"transition:"all 0.3s"backdropFilter:"blur(10px)"}}
 >
 {refreshing? (
 ...
 to="/"
 style={{
 fontSize:"12px"color:"#94a3b8"textDecoration:"none"transition:"color 0.3s"}}
 >
 {t("suivi.back_home_arrow")}
 </Link>
 </div>
