...
function ReservationPage() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[var(--gold-foreground)] text-foreground">
      <HeadMeta lang={lang} title={title} desc={desc} />

      {/* Header */}
      <header className="flex items-center justify-between border-b border-primary/20 bg-[var(--gold-foreground)]/90 px-4 py-3 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <Link to="/" className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-primary/30 text-primary transition hover:bg-primary/10">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="font-display text-lg font-semibold leading-tight text-primary">{t("reserver.chat.title")}</h1>
            <p className="text-xs text-foreground/60">{t("common.available_247")}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <a href={`tel:${lang === "en" ? "+33650260015" : "0650260015"}`} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-primary/30 text-primary transition hover:bg-primary/10" aria-label={t("nav.contact")}>
            <Phone className="h-4 w-4" />
          </a>
          <button
            onClick={() => setLang(lang === "en" ? "fr" : "en")}
            className="inline-flex h-9 items-center justify-center rounded-full border border-primary/30 px-3 text-xs font-medium text-primary transition hover:bg-primary/10"
          >
            {lang === "en" ? "FR" : "EN"}
          </button>
        </div>
        ...
        <div className="mx-auto max-w-2xl space-y-4">
              ...
              >
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                    m.role === "user"
                      ? "rounded-br-sm bg-primary text-[var(--gold-foreground)]"
                      : "rounded-bl-sm border border-foreground/10 bg-muted text-foreground"
                  }`}
                >
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="rounded-2xl rounded-bl-sm border border-foreground/10 bg-muted px-4 py-3">
          ...
          {confirmed && (
            ...
            >
              <p className="mb-2 text-sm font-medium text-primary">✅ {t("reserver.chat.booking_ok")}</p>
              <p className="mb-3 text-xs text-foreground/80">{t("reserver.chat.booking_ref")}: <span className="font-mono">{confirmed.suiviId}</span></p>
              <div className="flex flex-wrap gap-2">
                <a href={confirmed.trackingLink} className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-medium text-[var(--gold-foreground)]">
                  <MapPin className="h-4 w-4" /> {t("reserver.chat.booking_track")}
                </a>
                <Link to="/" className="inline-flex items-center gap-2 rounded-full border border-primary/30 px-4 py-2 text-sm text-primary">
      ...
      {/* Quick actions */}
      {messages.length < 2 && (
        <div className="border-t border-foreground/5 bg-[var(--gold-foreground)] px-4 py-3">
          <div className="mx-auto max-w-2xl">
            <p className="mb-2 text-xs text-foreground/50">{lang === "en" ? "Quick starts" : "Démarrages rapides"}</p>
            <div className="flex flex-wrap gap-2">
              {quickActions.map((a) => (
                <button
                  key={a.label}
                  onClick={() => sendQuick(a.text)}
                  className="inline-flex items-center gap-1.5 rounded-full border border-primary/20 bg-muted px-3 py-1.5 text-xs text-foreground/90 transition hover:border-primary/40 hover:text-primary"
                >
      ...
      {/* Composer */}
      <div className="border-t border-primary/20 bg-[var(--gold-foreground)] p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          ...
          <Input
            ...
            disabled={isLoading}
            className="flex-1 rounded-full border-foreground/10 bg-muted px-4 py-3 text-sm text-foreground placeholder:text-foreground/40 focus-visible:border-primary/50 focus-visible:ring-primary/20"
          />
          <Button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-primary p-0 text-[var(--gold-foreground)] hover:bg-primary/90 disabled:opacity-40"
...
function WelcomeBubble({ t, lang }: { t: (k: string) => string; lang: string }) {
  return (
    <div className="flex justify-start">
      <div className="max-w-[90%] rounded-2xl rounded-bl-sm border border-foreground/10 bg-muted px-4 py-3 text-sm leading-relaxed text-foreground/90">
        <p className="mb-1 font-medium text-primary">Margot</p>
        ...
        if (part.type === "tool-invocation") {
          ...
          if (ti.state === "result") {
            if (ti.toolName === "compute_quote" && ti.result?.prix_estime) {
              return (
                <div key={idx} className="rounded-xl bg-[var(--gold-foreground)]/60 p-2 text-xs">
                  <p className="text-primary">💶 {ti.result.prix_estime.toFixed(2)} €</p>
                  <p className="text-foreground/60">{ti.result.distance_km} km · {ti.result.duree_min} min</p>
                </div>
              );
            }
            ...
            if (ti.toolName === "human_handoff") {
              return (
                <div key={idx} className="space-y-1 text-xs">
                  ...
                  <p><a href={`tel:${ti.result.phone?.split("/")[0]?.trim()}`} className="text-primary">{ti.result.phone}</a></p>
                </div>
              );
            }
          }
