...
export const Route = createFileRoute("/api/public/hooks/recompute-durations-tick")({
 ...
 const { data: rows, error } = await supabaseAdmin.from("reservations").select("id,depart,arrivee,destination,duree_s,status,pickup_datetime").is("duree_recomputed_at" as any, null).not("status""in""(completed,cancelled,annulee)").order("pickup_datetime"{ ascending: false }).limit(BATCH_SIZE);
 ...
 if (!depart ||!arrivee) {
 await supabaseAdmin.from("reservations").update({ duree_recomputed_at: stamp } as any).eq("id"row.id);
 results.push({
 id: row.id,
 status:"skipped"previous_duree_s: row.duree_s?? null,
 new_duree_s: null,
 ...
 results.push({
 id: row.id,
 status:"unchanged"previous_duree_s: oldDureeS,
 new_duree_s: newDureeS,
 });
 } else {
 const { error: upErr } = await supabaseAdmin.from("reservations").update({ duree_s: newDureeS, duree_recomputed_at: stamp } as any).eq("id"row.id);
 if (upErr) throw new Error(upErr.message);
 results.push({
 id: row.id,
 status:"updated"previous_duree_s: oldDureeS,
 new_duree_s: newDureeS,
 });
 }
 } catch (e) {
 ...
 try {
 await supabaseAdmin.from("reservations").update({ duree_recomputed_at: new Date().toISOString() } as any).eq("id"row.id);
 } catch {
 /* noop */
 }
 results.push({
 id: row.id,
 status:"error"previous_duree_s: row.duree_s?? null,
 new_duree_s: null,
