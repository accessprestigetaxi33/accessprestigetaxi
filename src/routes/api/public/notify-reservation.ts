...
export const Route = createFileRoute("/api/public/notify-reservation")({
 ...
 const BASE_COLUMNS ="id, nom, client_name, telephone, client_phone, email, pickup_datetime, depart, arrivee, destination, passagers, bagages, service_type, suivi_id, client_account_id, assigned_driver";
 ...
 const withLang = await supabase.from("reservations").select(`${BASE_COLUMNS}, lang`).eq("id"reservationId).maybeSingle();
 if (withLang.error && (withLang.error as any).code ==="42703") {
 console.warn("[notify-reservation] colonne'lang' absente sur reservations — fallback fr." +"Ajouter une migration: ALTER TABLE reservations ADD COLUMN lang text DEFAULT'fr';");
 const withoutLang = await supabase.from("reservations").select(BASE_COLUMNS).eq("id"reservationId).maybeSingle();
 ...
 title: assignedLabel? `🚕 Nouvelle résa — ${assignedLabel}`:"🚕 Nouvelle résa"body: `${clientName} — ${trajet}`,
 url:"/driver"tag: `chauffeur-res-${reservationId}`,
 ...
 const clientResult = await sendPushToAudience("client"{
