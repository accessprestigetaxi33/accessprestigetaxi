...
export const Route = createFileRoute("/api/public/reviews")({
 ...
 const { data, error } = await supabase.from("avis").select("id,status").eq("reservation_id"parsed.data).limit(1).maybeSingle();
 ...
 supabase.from("avis").select(columns).eq("status""approved").order("created_at"{ ascending: false }).limit(20),
 ...
 const { data: existing, error: existingError } = await supabase.from("avis").select("id,status").eq("reservation_id"data.reservation_id).limit(1).maybeSingle();
 ...
 const { data: reservation } = await supabase.from("reservations").select("client_name,nom").eq("id"data.reservation_id).maybeSingle();
 authorName = (reservation?.client_name || reservation?.nom ||"Client").trim();
 }

 const { data: inserted, error } = await supabase.from("avis").insert({
 reservation_id: data.reservation_id,
 author_name: authorName.slice(0, 80) ||"Client"note: data.note,
 commentaire: data.commentaire?.trim() || null,
 status:"pending"}).select("id,status").single();
 if (error) return Response.json({ error: error.message }, { status: 500 });
 ...
 url:"/driver"tag: `new-review-${inserted.id}`,
 });
 } catch (error) {
