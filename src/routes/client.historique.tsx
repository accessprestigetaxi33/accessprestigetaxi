...
function ClientHistorique() {
 ...
 useEffect(() => {
 if (!session) return;
 const channel = supabase.channel("historique-realtime").on("postgres_changes"{ event:"UPDATE"schema:"public"table:"reservations" }, () => refresh()).subscribe();
 return () => {
 ...
 style={{
 background: isCompleted?"rgba(148,163,184,0.18)":"rgba(239,68,68,0.18)"color: isCompleted?"#cbd5e1":"#fca5a5"}}
 >
 {isCompleted? t("client.historique.completed"): r.status ==="cancelled"? t("client.historique.cancelled"): t("client.historique.refused")}
 </span>
 </div>
