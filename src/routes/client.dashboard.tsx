...
const COPY = {
 fr: {
 eyebrow:"Espace client"hello: (n: string) => `Bonjour ${n} 👋`,
 activeRide:"Course en cours"track:"Suivre en direct"details:"Détails"book:"Réserver une course"call: (n: string, p: string) => `Appeler ${n} — ${p}`,
 trips:"Mes trajets actifs"history:"Historique des courses"chat:"Écrire à mon chauffeur"profile:"Mon profil"backSite:"← Site"you:"vous"},
 en: {
 eyebrow:"Client area"hello: (n: string) => `Hello ${n} 👋`,
 activeRide:"Ride in progress"track:"Track live"details:"Details"book:"Book a ride"call: (n: string, p: string) => `Call ${n} — ${p}`,
 trips:"My active rides"history:"Ride history"chat:"Message my driver"profile:"My profile"backSite:"← Website"you:"there"},
} as const;
 ...
 * { -webkit-tap-highlight-color: transparent; box-sizing: border-box; touch-action: manipulation; }
 ...
 font-family:'DM Sans'-apple-system, BlinkMacSystemFont'Segoe UI'sans-serif; }.cd-root { position: fixed; inset: 0; max-width: 640px; margin: 0 auto;
 display: flex; flex-direction: column;
 background: linear-gradient(180deg, #0a0a0a 0%, #111827 100%); }.cd-header { flex-shrink: 0; display: flex; align-items: center; justify-content: space-between;
 padding: 12px 16px; background: rgba(10,10,10,0.92);
 backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
 border-bottom: 1px solid rgba(255,255,255,0.06); }.cd-scroll { flex: 1; overflow-y: auto; -webkit-overflow-scrolling: touch; padding: 20px 16px 0; }
`;
...
function ClientDashboard() {
 ...
 setLoading(true);
 listClientReservations({ data: { token: session.token } }).then(setRows).catch(() => {}).finally(() => setLoading(false));
 }, [ready, session]);
 ...
 const channel = supabase.channel("cd-home-status").on("postgres_changes"{ event:"UPDATE"schema:"public"table:"reservations"filter: `id=in.(${ids.join("")})` },
 () => {
 if (!session) return;
 listClientReservations({ data: { token: session.token } }).then(setRows).catch(() => {});
 },
 ).subscribe();
 return () => {
 ...
 fontSize: 12,
 fontWeight: 600,
 color:"rgba(255,255,255,0.4)"textDecoration:"none"padding:"6px 10px"borderRadius: 8,
 ...
 left:"50%"transform:"translateX(-50%)"width: 300,
 height: 300,
 borderRadius:"50%"pointerEvents:"none"background:"radial-gradient(circle, rgba(201,168,76,0.15) 0%, transparent 70%)"filter:"blur(40px)"}}
 />
 ...
 <div
 style={{
 background:"rgba(232,201,109,0.07)"border:"1px solid rgba(232,201,109,0.22)"borderRadius: 14,
 padding:"14px 16px"}}
 >
 <div
 ...
 fontSize: 12,
 fontWeight: 700,
 color:"#E8C96D"textDecoration:"none"padding:"6px 12px"borderRadius: 8,
 background:"rgba(232,201,109,0.12)"border:"1px solid rgba(232,201,109,0.25)"}}
 >
 ...
 fontSize: 12,
 fontWeight: 600,
 color:"rgba(255,255,255,0.6)"textDecoration:"none"padding:"6px 12px"borderRadius: 8,
 border:"1px solid rgba(255,255,255,0.1)"}}
 >
 {c.details}
 </Link>
 ...
 background:"linear-gradient(135deg, #C9A84C 0%, #E8C96D 100%)"color:"#000"fontWeight: 700,
 fontSize: 14,
 textDecoration:"none"}}
 >
 ...
 display:"flex"alignItems:"center"justifyContent:"center"gap: 8,
 padding:"14px"borderRadius: 14,
 border:"1px solid rgba(255,255,255,0.1)"background:"rgba(255,255,255,0.04)"color:"#fff"fontWeight: 600,
 fontSize: 14,
 textDecoration:"none"}}
 >
 ...
 {
 to:"/client/historique"icon: <History style={{ width: 16, height: 16 }} />,
 label: c.history,
 },
 ...
 to={item.to}
 style={{
 display:"flex"alignItems:"center"justifyContent:"space-between"padding:"13px 14px"borderRadius: 12,
 border:"1px solid rgba(255,255,255,0.07)"background:"rgba(255,255,255,0.03)"color:"#fff"textDecoration:"none"fontSize: 13,
 fontWeight: 600,
 }}
 >
