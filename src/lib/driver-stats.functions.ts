...
export const getDriverStats = createServerFn({ method:"POST" }).inputValidator((input: unknown) => TokenSchema.extend({ days: z.number().int().min(1).max(365).optional() }).parse(input)).handler(async ({ data }) => {
 ...
 supabaseAdmin.from("reservations").select("id,status,assigned_driver,prix_estime,distance_km,created_at,duree_s").gte("created_at"since),
 supabaseAdmin.from("reservation_events").select("reservation_id,event_type,to_value,driver,created_at").gte("created_at"since).order("created_at"{ ascending: true }),
...
export const listReservationEvents = createServerFn({ method:"POST" }).inputValidator((input: unknown) =>
 TokenSchema.extend({
 ...
 let q = supabaseAdmin.from("reservation_events").select("id,reservation_id,event_type,from_value,to_value,driver,client_name,depart,destination,created_at").order("created_at"{ ascending: false }).limit(data.limit?? 60);
