...
const HINTS_FR: Record<Exclude<AddressReason"OK">, string> = {
 INPUT_EMPTY:"Demande poliment au client l'adresse manquante en une phrase naturelle."INPUT_TOO_SHORT:"Demande poliment quelques précisions supplémentaires (numéro, rue, ville, ou un point de repère connu)."NO_MATCH:"Demande poliment au client de reformuler l'adresse avec un repère plus clair (numéro + rue + ville, ou un lieu connu comme Aéroport La Rochelle, Gare de La Rochelle, Vieux-Port, Zoo de La Palmyre, Aquarium de La Rochelle, Île de Ré, Île d'Oléron, Fort Boyard, plage de Royan…)."LOW_CONFIDENCE:"Reformule ce que tu as compris et demande une confirmation courte au client."};

const HINTS_EN: Record<Exclude<AddressReason"OK">, string> = {
 INPUT_EMPTY:"Politely ask the client for the missing address in one natural sentence."INPUT_TOO_SHORT:"Politely ask for a few more details (street number, street, city, or a known landmark)."NO_MATCH:"Politely ask the client to rephrase the address with a clearer landmark (street number + street + city, or a known place like La Rochelle Airport, La Rochelle train station, Old Port, La Palmyre Zoo, Aquarium, Île de Ré, Île d'Oléron, Fort Boyard, Royan beach…)."LOW_CONFIDENCE:"Rephrase what you understood and ask the client for a short confirmation."};
...
function preNormalize(raw: string): string {
 return raw.replace(/[\u200B-\u200D\uFEFF]/g"") // zero-width.replace(/\s+/g"").trim();
}
...
export async function resolveAddress(
 ...
 role:"depart" |"arrivee"lang ="fr"): Promise<ResolvedAddress> {
 ...
 return {
 ok: false,
 reason:"INPUT_TOO_SHORT"hint: hints(lang).INPUT_TOO_SHORT,
 input,
 };
 }
 try {
...
const FALLBACK_REPLY_FR ="Un instant s'il vous plaît. Pourriez-vous reformuler votre demande, je souhaite être sûre de bien vous répondre?";
const FALLBACK_REPLY_EN ="One moment please. Could you rephrase your request? I want to make sure I answer you correctly.";
...
export function sanitizeAssistantReply(raw: string | null | undefined, lang ="fr"): string {
 ...
 if (mutated) {
 console.warn("[assistant-sanitize] leaked_tokens_removed original="JSON.stringify(original)"cleaned="JSON.stringify(cleaned),
 );
 }
