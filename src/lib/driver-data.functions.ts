...
export const driverUpdateReservation = createServerFn({ method:"POST" }).inputValidator((input: unknown) =>
 TokenSchema.extend({
 ...
 let q = supabaseAdmin.from("reservations").update(data.patch as any).eq("id"data.reservation_id);
...
export const driverListReservations = createServerFn({ method:"POST" }).inputValidator((input: unknown) =>
 ...
 const { data: rows, error } = await supabaseAdmin.from("reservations").select("id,depart,destination,pickup_datetime,status,prix_estime,distance_km,assigned_driver").gte("pickup_datetime"today.toISOString()).lt("pickup_datetime"tomorrow.toISOString()).neq("status""cancelled").order("pickup_datetime"{ ascending: true });
 if (error) throw new Error(error.message);
 return { rows: rows?? [], clients: [] as any[] };
 }

 const [{ data: rows, error }, { data: clients }] = await Promise.all([
 supabaseAdmin.from("reservations").select("client_name,client_phone,depart,destination,prix_estime,pickup_datetime,status").not("client_phone""is"null).order("pickup_datetime"{ ascending: false }),
...
export const driverDeleteClient = createServerFn({ method:"POST" }).inputValidator((input: unknown) =>
 TokenSchema.extend({
 ...
 const ids = ((all as any[])?? []).filter((r) => normalize(r.client_phone??"") === target || normalize(r.telephone??"") === target).map((r) => r.id);
