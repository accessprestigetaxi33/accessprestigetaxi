...
export const subscribePush = createServerFn({ method:"POST" }).inputValidator((input) => subSchema.parse(input)).handler(async ({ data }) => {
...
export const unsubscribePush = createServerFn({ method:"POST" }).inputValidator((input) => z.object({ fcm_token: z.string().min(10).max(500) }).parse(input)).handler(async ({ data }) => {
 const { getTaxiSupabaseAdmin } = await import("@/lib/taxi-supabase.server");
 const supabaseAdmin = getTaxiSupabaseAdmin();
 await supabaseAdmin.from("push_subscriptions").delete().eq("fcm_token"data.fcm_token);
 return { ok: true };
...
export const sendTestPush = createServerFn({ method:"POST" }).inputValidator((input) => z.object({ audience: z.enum(["chauffeur""client"]) }).parse(input)).handler(async ({ data }) => {
 const { sendPushToAudience } = await import("@/lib/push.server");
 return sendPushToAudience(data.audience, {
 title:"🔔 Test notification"body: `Notification test envoyée à l'audience « ${data.audience} ».`,
 url: data.audience ==="client"?"/":"/driver"tag:"test-push"});
 });
...
export const notifyNewReview = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 ...
 url:"/driver"tag: `new-review-${Date.now()}`,
 });
 });
...
export const notifyNewReservation = createServerFn({ method:"POST" }).inputValidator((input) => z.object({ reservation_id: z.string().uuid() }).parse(input)).handler(async ({ data }) => {
 ...
 const { data: r, error: fetchErr } = await supabaseAdmin.from("reservations").select("id, nom, client_name, client_phone, telephone, client_email, email, depart, arrivee, destination, pickup_datetime, nb_passagers, passagers, bagages, service_type, suivi_id, lang").eq("id"data.reservation_id).maybeSingle();
 if (fetchErr) {
 ...
 const emailPayload = {
 templateName:"new-reservation-admin"recipientEmail:"taxi.city033@gmail.com"idempotencyKey: `new-res-admin-${r.id}`,
 templateData: {
 id: r.id,
 nom: clientName,
 client_name: clientName,
 phone: r.client_phone || r.telephone ||""telephone: r.client_phone || r.telephone ||""email: r.client_email || r.email ||""depart: r.depart,
 arrivee: r.arrivee || r.destination ||"—"destination: r.arrivee || r.destination ||"—"pickup_datetime: r.pickup_datetime??""passagers: r.nb_passagers || r.passagers || 1,
 bagages: r.bagages?? 0,
 service_type: (r as any).service_type??""admin_url: `${APP_URL}/driver`,
 },
 };
 ...
 method:"POST"headers: {"Content-Type":"application/json"...(serviceKey? { Authorization: `Bearer ${serviceKey}` }: {}),
 },
 ...
 templateName:"reservation-client-confirmation"recipientEmail: clientEmail,
 idempotencyKey: `new-res-client-${r.id}`,
 templateData: {
 lang: (r as any).lang ||"fr"nom: clientName,
 pickup_datetime: r.pickup_datetime??""depart: r.depart,
 arrivee: r.arrivee || r.destination ||"—"passagers: r.nb_passagers || r.passagers || 1,
 bagages: r.bagages?? 0,
 reservation_id: r.id,
 ...
 console.log("[notifyNewReservation] sending client confirmation email via bridge →"`${APP_URL}/lovable/email/transactional/send`,
 );
...
async function computeEtaMinutes(_reservationId: string, depart: string): Promise<number | null> {
 ...
 const { getTaxiSupabaseAdmin } = await import("@/lib/taxi-supabase.server");
 const supabaseAdmin = getTaxiSupabaseAdmin();
 const { data: gps } = await supabaseAdmin.from("driver_gps" as any).select("latitude, longitude, captured_at, updated_at").order("updated_at"{ ascending: false, nullsFirst: false }).limit(1).maybeSingle();
...
export const notifyReservationStatus = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 ...
 const supabaseAdmin = getTaxiSupabaseAdmin();
 const { data: r, error: fetchErr } = await supabaseAdmin.from("reservations").select("id, nom, client_name, client_phone, telephone, depart, arrivee, destination, suivi_id, lang, client_account_id").eq("id"data.reservation_id).maybeSingle();
 if (fetchErr) {
 ...
 const { error: updateError } = await supabaseAdmin.from("reservations").update({ status: data.status, updated_at: new Date().toISOString() }).eq("id"r.id);
 if (updateError) {
 ...
 clientResult = await sendPushToAudience("client"{
 title:"✅ Course confirmée"body: `Patricia a confirmé votre course: ${trajet}.`,
 url,
 ...
 clientResult = await sendPushToAudience("client"{
 title:"🚖 Votre taxi arrive"body: `Votre taxi arrive vers ${r.depart}${etaTxt}.`,
 url,
 ...
 clientResult = await sendPushToAudience("client"{
 title:"✅ Votre taxi est arrivé"body: `Votre chauffeur vous attend au point de prise en charge.`,
 url,
 ...
 clientResult = await sendPushToAudience("client"{
 title:"🏁 Course terminée"body:"Merci pour votre trajet avec Access Prestige Taxi."url,
...
export const updateReservationRoute = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 reservation_id: z.string().uuid(),
 ...
 const { data: r, error: fetchErr } = await supabaseAdmin.from("reservations").select("id, suivi_id, pickup_datetime, prix_estime, distance_km, nom, client_name, lang, status").eq("id"data.reservation_id).maybeSingle();
 ...
 const { error: updErr } = await supabaseAdmin.from("reservations").update({
...
export const listPushFailures = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 ...
 const { getTaxiSupabaseAdmin } = await import("@/lib/taxi-supabase.server");
 const supabaseAdmin = getTaxiSupabaseAdmin();
 let q = supabaseAdmin.from("push_send_failures").select("id, created_at, audience, tag, reservation_id, fcm_token_suffix, http_status, error_code, title, body, user_agent").order("created_at"{ ascending: false }).limit(data.limit?? 200);
