 ...
 */
export const clientRequestPasswordReset = createServerFn({ method:"POST" }).inputValidator((input: unknown) => RequestSchema.parse(input)).handler(async ({ data }): Promise<{ ok: true }> => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

 const { data: account } = await supabaseAdmin.from("client_accounts").select("id, email, client_name").eq("email"data.email).maybeSingle();
 ...
 await supabaseAdmin.from("client_password_resets" as any).update({ used_at: new Date().toISOString() }).eq("client_account_id"account.id).is("used_at"null);
...
<div style="max-width:560px;padding:24px;margin:0 auto;">
 ...
 template_name:"client-password-reset"recipient_email: account.email,
 status:"pending"});
 await supabaseAdmin.rpc("enqueue_email" as any, {
 queue_name:"transactional_emails"payload: {
 message_id: messageId,
 to: account.email,
 from:"Access Prestige Taxi <noreply@notify.accessprestigetaxi.lovable.app>"reply_to:"taxi.city033@gmail.com"sender_domain:"notify.accessprestigetaxi.lovable.app"subject,
 html,
 text,
 purpose:"transactional"label:"client-password-reset"idempotency_key: messageId,
 ...
 */
export const clientPerformPasswordReset = createServerFn({ method:"POST" }).inputValidator((input: unknown) => ResetSchema.parse(input)).handler(async ({ data }): Promise<{ ok: true }> => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

 const tokenHash = await sha256Hex(data.token);
 const { data: row } = await supabaseAdmin.from("client_password_resets" as any).select("id, client_account_id, expires_at, used_at").eq("token_hash"tokenHash).maybeSingle();
 ...
 const { error: upErr } = await supabaseAdmin.from("client_account_secrets" as any).upsert({ client_account_id: r.client_account_id, password_hash: hash, updated_at: new Date().toISOString() });
 if (upErr) throw new Error("UPDATE_FAILED");

 await supabaseAdmin.from("client_password_resets" as any).update({ used_at: new Date().toISOString() }).eq("id"r.id);
