...
export async function sendDriverPush(
 ...
 const { data: subs } = await supabaseAdmin.from("push_subscriptions").select("endpoint, p256dh, auth, fcm_token, audience").eq("audience""driver").eq("user_id"driver as any).limit(20);
 ...
 const tokens = subs.map((s) => s.fcm_token).filter((t): t is string => typeof t ==="string" && t.length > 10);
...
export async function enqueueClientConfirmationEmail(
 ...
 queue_name:"transactional"payload: {
 template_name: lang ==="en"?"reservation-confirmed-en":"reservation-confirmed-fr"to: email,
