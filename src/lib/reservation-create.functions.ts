...
export const createReservationPublic = createServerFn({ method:"POST" }).inputValidator((input: unknown) => CreateSchema.parse(input)).handler(async ({ data }) => {
 ...
 status:"pending"suivi_id: data.suivi_id,
 ...
 const { data: inserted, error } = await supabaseAdmin.from("reservations").insert(payload).select("id,suivi_id").single();
...
export const getTaxiAvailability = createServerFn({ method:"GET" }).handler(async () => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { count, error } = await supabaseAdmin.from("reservations").select("id"{ count:"exact"head: true }).in("status"["accepted""en_route""arrived"]);
 if (error) throw new Error(error.message);
