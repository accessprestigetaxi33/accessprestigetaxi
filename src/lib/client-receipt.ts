...
export function downloadReceiptPDF(
 ...
 doc.text("Access Prestige Taxi — taxi.city033@gmail.com — 06 50 26 00 15"W / 2,
 800,
 { align:"center" },
 );
...
export function exportReservationsCSV(rows: ClientReservation[]) {
 const headers = ["Date""Reference""Depart""Arrivee""Passagers""Bagages""Statut""Montant_EUR""Paiement"];
 ...
 String(r.bagages?? 0),
 r.status,
 r.prix_estime!= null? Number(r.prix_estime).toFixed(2):""r.paiement ||""].map((v) => `"${String(v).replace(/"/g'""')}"`);
