...
async function getAccessToken(): Promise<string> {
 ...
 const claim = {
 iss: sa.client_email,
 scope:"https://www.googleapis.com/auth/firebase.messaging"aud: tokenUri,
 exp: now + 3600,
 iat: now,
 };
 ...
 const cryptoKey = await crypto.subtle.importKey("pkcs8"keyBuf,
...
async function sendFcmToToken(
 ...
 const relativeUrl =
 payload.url &&!/^https?:\/\//i.test(payload.url)? payload.url.startsWith("/")? payload.url: `/${payload.url}`: clickUrl;
 const extraData = {
 url: relativeUrl,
 click_url: clickUrl,
 tag: payload.tag ||"taxi-fcm"audience,...(reservationId? { reservation_id: reservationId }: {}),
 };
...
export async function sendPushToAudience(
 ...
 let q = supabaseAdmin.from("push_subscriptions").select("id, fcm_token, user_agent, last_seen_at, created_at").eq("audience"audience).not("fcm_token""is"null);
