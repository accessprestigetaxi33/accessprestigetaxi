...
export const verifyDriverToken = createServerFn({ method:"POST" }).inputValidator((input: unknown) => TokenSchema.parse(input)).handler(async ({ data }) => {
...
export const getActiveVisitorCount = createServerFn({ method:"POST" }).inputValidator((input: unknown) =>
