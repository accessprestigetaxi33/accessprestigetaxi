  ...
  .handler(async ({ data }) => {
    ...
    const [drivers, rotation, events, reservations] = await Promise.all([
      supabaseAdmin
        ...
        .select("id,name,phone,active,sort_order,updated_at")
      ...
      supabaseAdmin
        ...
        .limit(data.eventLimit ?? 100),
      supabaseAdmin
        .from("reservations")
        .select("id,nom,client_name,telephone,depart,destination,arrivee,pickup_datetime,status,assigned_driver,prix_estime,created_at")
...
export const adminSetDriverActive = createServerFn({ method: "POST" })
  .inputValidator((input: unknown) =>
...
export const adminSetRotation = createServerFn({ method: "POST" })
  .inputValidator((input: unknown) =>
  ...
  .handler(async ({ data }) => {
    ...
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await supabaseAdmin
      .from("driver_rotation" as any)
      .update({ last_driver: data.last_driver, updated_at: new Date().toISOString() } as any)
      .eq("id", 1);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const adminFixReservation = createServerFn({ method: "POST" })
  .inputValidator((input: unknown) =>
    TokenSchema.extend({
