...
const CANONICAL_PLACES: Array<{ match: RegExp; label: string; lat: number; lng: number }> = [
  {
    match:
      /(^|\s)(aeroport|airport|lrh)(\s|$).*(la\s+rochelle|ile\s+de\s+re|iledere|ile\s+dere)|^lrh$|^aeroport$|^airport$/,
  ...
  {
    match:
      /(\broyan\s+plage\b|\bplage\s+royan\b|\bsaint\s+georges\s+de\s+didonne\b|\bsaint\s+georges\b)/,
...
const OUTSIDE_CANONICAL_PLACES: Array<{ match: RegExp; label: string; lat: number; lng: number }> =
  [
    ...
    {
      match:
        /gare.*(saint\s*jean|st\s*jean).*bordeaux|bordeaux.*gare.*(saint\s*jean|st\s*jean)|^gare\s+(saint|st)\s*jean$|charles\s+domercq/,
    ...
    {
      match:
        /grand\s+theatre.*bordeaux|bordeaux.*grand\s+theatre|place\s+de\s+la\s+comedie.*bordeaux/,
    ...
    {
      match:
        /(palais\s+des\s+congres|parc\s+des\s+expositions).*bordeaux|bordeaux.*parc\s+des\s+expositions/,
    ...
    {
      match: /\bsaint\s*michel\b.*bordeaux|bordeaux.*saint\s*michel/,
      label: "Quartier Saint-Michel, 33800 Bordeaux",
      lat: 44.8329,
      lng: -0.5665,
    },
    {
      match: /\bsaint\s*pierre\b.*bordeaux/,
      label: "Quartier Saint-Pierre, 33000 Bordeaux",
      lat: 44.8397,
      lng: -0.5714,
    },
    { match: /\bnansouty\b/, label: "Nansouty, 33800 Bordeaux", lat: 44.8228, lng: -0.5747 },
    {
      match: /\bcaudéran\b|\bcauderan\b/,
      label: "Caudéran, 33200 Bordeaux",
      lat: 44.8459,
      lng: -0.6112,
    },
    {
      match: /\bbastide\b.*bordeaux|bordeaux.*bastide/,
      label: "La Bastide, 33100 Bordeaux",
      lat: 44.8419,
      lng: -0.5548,
    },
    {
      match: /\bginko\b|berges\s+du\s+lac/,
      label: "Ginko / Bordeaux Lac, 33300 Bordeaux",
      lat: 44.8863,
      lng: -0.5769,
    },
    {
      match: /\beuratlantique\b/,
      label: "Bordeaux Euratlantique, 33800 Bordeaux",
      lat: 44.8231,
      lng: -0.5527,
    },
    ...
    {
      match: /\ble\s+bouscat\b|\bbouscat\b/,
      label: "Le Bouscat, 33110",
      lat: 44.8654,
      lng: -0.5989,
    },
    {
      match: /\bbruges\b.*(33|bordeaux)|^bruges$/,
      label: "Bruges, 33520",
      lat: 44.8783,
      lng: -0.6003,
    },
    ...
    {
      match: /\bvillenave\s+d\s*ornon\b/,
      label: "Villenave-d'Ornon, 33140",
      lat: 44.7803,
      lng: -0.5661,
    },
    {
      match: /\bsaint\s*medard\s+en\s+jalles\b/,
      label: "Saint-Médard-en-Jalles, 33160",
      lat: 44.8969,
      lng: -0.7169,
    },
    ...
    {
      match: /^bordeaux$|bordeaux\s+centre|centre\s+ville\s+bordeaux/,
    ...
    {
      match: /dune\s+du\s+pilat|dune\s+du\s+pyla/,
      label: "Dune du Pilat, 33115 La Teste-de-Buch",
      lat: 44.5892,
      lng: -1.2136,
    },
    {
      match: /gare.*angouleme|angouleme.*gare/,
      label: "Gare d'Angoulême, 16000 Angoulême",
      lat: 45.6552,
      lng: 0.1637,
    },
    {
      match: /gare.*niort|niort.*gare/,
      label: "Gare de Niort, 79000 Niort",
      lat: 46.3172,
      lng: -0.4636,
    },
    {
      match: /gare.*poitiers|poitiers.*gare/,
      label: "Gare de Poitiers, 86000 Poitiers",
      lat: 46.5822,
      lng: 0.3336,
    },
    {
      match: /(aeroport|airport).*nantes|nantes\s+atlantique/,
      label: "Aéroport Nantes-Atlantique (NTE), 44340 Bouguenais",
      lat: 47.1532,
      lng: -1.6108,
    },
    {
      match: /(aeroport|airport).*(roissy|charles\s+de\s+gaulle)|\bcdg\b/,
      label: "Aéroport Paris-Charles-de-Gaulle (CDG), 95700 Roissy",
      lat: 49.0097,
      lng: 2.5479,
    },
    {
      match: /(aeroport|airport).*orly|\bory\b/,
      label: "Aéroport Paris-Orly (ORY), 94390 Orly",
      lat: 48.7233,
      lng: 2.3794,
    },
    {
      match: /gare\s+montparnasse/,
      label: "Gare Montparnasse, 75015 Paris",
      lat: 48.8412,
      lng: 2.32,
    },
  ];
...
function normalize(q: string): string[] {
  ...
  if (/aeroport|airport/.test(lower)) {
    if (
      /la\s+rochelle|ile\s+de\s+re|iledere/.test(lower) ||
      lower === "aeroport" ||
      lower === "airport"
    ) {
      variants.push("Aéroport La Rochelle-Île de Ré");
    } else {
      const cityToken = lower
        .replace(/\b(aeroport|airport|de|du|d|l|la|le)\b/g, " ")
        .trim()
        .split(/\s+/)[0];
  ...
  if (/\bgare\b/.test(lower)) {
    if (/la\s+rochelle|rochelle/.test(lower) || lower === "gare") {
      ...
      const rest = lower
        .replace(/\b(gare|sncf|de|du|d|la|le|les)\b/g, " ")
        .replace(/\s+/g, " ")
        .trim();
  ...
  const hasHint =
    /la\s+rochelle|royan|saintes|rochefort|oleron|ile\s+de\s+re|iledere|chatelaillon|châtelaillon|saint\s+georges|la\s+palmyre|fort\s+boyard|charente\s*maritime|17\d{3}/i.test(
      cleaned,
    );
  if (!hasHint) {
