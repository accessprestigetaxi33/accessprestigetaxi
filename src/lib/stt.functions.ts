 ...
 */
export const transcribeAudio = createServerFn({ method:"POST" }).inputValidator((data: { base64: string; mime: string; lang?: string }) => {
 ...
 const ext =
 mime.includes("wav")?"wav": mime.includes("mp4") || mime.includes("m4a")?"mp4": mime.includes("mpeg") || mime.includes("mp3")?"mp3": mime.includes("ogg")?"ogg":"webm";
