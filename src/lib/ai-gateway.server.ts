...
export function createLovableAiGatewayProvider(
 ...
 name:"lovable"baseURL:"https://ai.gateway.lovable.dev/v1"supportsStructuredOutputs: options?.structuredOutputs?? false,
...
export function getLovableAiGatewayResponseHeaders(
 ...
 (headers.get("Access-Control-Expose-Headers")??"").split("").map((header) => header.trim()).filter(Boolean),
 );
