...
- Horaires d'ouverture:. Pas de réservation en dehors de ces horaires.
...
function nowParis() {
 ...
 timeZone: TIMEZONE,
 year:"numeric"month:"2-digit"day:"2-digit"hour:"2-digit"minute:"2-digit"second:"2-digit"hourCycle:"h23"}).format(new Date()),
 );
}
...
function buildTools(lang: string, state: ReservationStateType, _gateway: LovableAiGatewayProvider) {
 ...
 compute_quote: tool({
 description:
 lang ==="en"?"Compute a fare estimate between two addresses for a given pickup time and passenger count.":"Calcule un devis entre deux adresses pour une heure et un nombre de passagers donnés."inputSchema: zodSchema(
 z.object({
 ...
 return {
 error:
 lang ==="en"?"Unable to calculate the route. Please check the addresses or contact us.":"Impossible de calculer l'itinéraire. Vérifiez les adresses ou contactez-nous."} as const;
 }
 ...
 check_slot: tool({
 description:
 lang ==="en"?"Check whether a requested pickup slot is available and within opening hours.":"Vérifie si un créneau demandé est disponible et dans les horaires d'ouverture."inputSchema: zodSchema(
 z.object({
 ...
 available: false,
 reason:
 lang ==="en"? `Reservations must be made at least ${MIN_ADVANCE_MINUTES} minutes in advance.`: `Les réservations doivent être faites au moins ${MIN_ADVANCE_MINUTES} minutes à l'avance.`,
 ...
 return {
 available: false,
 reason: lang ==="en"?"We are open.":"Nous sommes ouverts."next_available: formatISO(next),
 };
 }
 state.pickup_datetime = args.pickup_datetime;
 return {
 available: true,
 reason: lang ==="en"?"The slot is available.":"Le créneau est disponible."next_available: null,
 };
 },
 }),
 confirm_reservation: tool({
 description:
 lang ==="en"?"Confirm and create the reservation after collecting all required fields.":"Confirme et crée la réservation après avoir collecté tous les champs requis."inputSchema: zodSchema(
 z.object({
 ...
 ok: false,
 message:
 lang ==="en"?"Unable to calculate the route. Please check the addresses or contact us.":"Impossible de calculer l'itinéraire. Vérifiez les adresses ou contactez-nous."};
 }
 ...
 ok: false,
 message:
 lang ==="en"?"We are open. Please choose another time.":"Nous sommes ouverts. Choisissez un autre horaire."};
 }
 ...
 service_type:"standard"source:"chat"};
 ...
 message:
 lang ==="en"? `Your reservation is confirmed for ${formatPickupDateTime(when, lang)}. You can follow your taxi live at: ${trackingLink}`: `Votre réservation est confirmée pour le ${formatPickupDateTime(when, lang)}. Suivez votre taxi en direct ici: ${trackingLink}`,
 };
 } catch (err: any) {
 ...
 ok: false,
 message:
 lang ==="en"?"We could not finalize your reservation. Please call us or try again.":"Nous n'avons pas pu finaliser votre réservation. Appelez-nous ou réessayez."};
 }
 },
 }),
 human_handoff: tool({
 description: lang ==="en"?"Hand off to a human operator.":"Transfère vers un opérateur humain."inputSchema: zodSchema(z.object({ reason: z.string().nullable() })),
 execute: async (_args: { reason: string | null }) => {
 return {
 ok: true,
 message:
 lang ==="en"?"Patricia or Alain will take over. You can reach us at the numbers below or on WhatsApp.":"Patricia ou Alain reprend la main. Vous pouvez nous joindre aux numéros ci-dessous ou sur WhatsApp."phone: `${PHONE_PATRICIA} / ${PHONE_ALAIN}`,
