...
async function assertClientOwnsReservation(
 ...
 const { data: r } = await supabaseAdmin.from("reservations").select("id, client_account_id, client_phone, telephone, client_email, email").eq("id"reservationId).maybeSingle();
 if (!r) throw new Error("NOT_FOUND");
 if (r.client_account_id === identity.account_id) return;
 const phoneTail = normalizePhone(identity.phone);
 const matchPhone =!!phoneTail &&
 (normalizePhone((r as any).client_phone) === phoneTail || normalizePhone((r as any).telephone) === phoneTail);
 const matchEmail =!!identity.email &&
...
export const sendClientMessage = createServerFn({ method:"POST" }).inputValidator((input) => clientSendSchema.parse(input)).handler(async ({ data }) => {
 ...
 const { data: r } = await supabaseAdmin.from("reservations").select("client_name, nom, suivi_id").eq("id"data.reservation_id).maybeSingle();
 const clientName = (r as any)?.client_name || (r as any)?.nom ||"Client";

 const { data: row, error } = await supabaseAdmin.from("reservation_messages").insert({
 reservation_id: data.reservation_id,
 sender:"client"content: data.content,
 read_by_client: true,
 ...
 url:"/driver"tag: `chat-driver-resa-${data.reservation_id}-${Date.now()}`,
...
export const sendChauffeurMessage = createServerFn({ method:"POST" }).inputValidator((input) => sendSchema.parse(input)).handler(async ({ data }) => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

 // Récupère suivi_id + compte client pour construire l'URL et cibler la push
 const { data: resa } = await supabaseAdmin.from("reservations").select("suivi_id, client_account_id").eq("id"data.reservation_id).maybeSingle();
 const suiviId = (resa as any)?.suivi_id || data.reservation_id;
 const accountId = (resa as any)?.client_account_id || undefined;

 const { data: row, error } = await supabaseAdmin.from("reservation_messages").insert({
 reservation_id: data.reservation_id,
 sender:"chauffeur"content: data.content,
 ...
 title:"💬 Patricia a répondu à votre message"body: data.content.slice(0, 100),
 url: `/suivi/${suiviId}`,
...
export const listReservationMessages = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 reservation_id: z.string().uuid(),
 ...
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 let q = supabaseAdmin.from("reservation_messages").select("id,reservation_id,sender,content,read_by_client,read_by_chauffeur,created_at").eq("reservation_id"data.reservation_id).order("created_at"{ ascending: false }).limit(data.limit?? 30);
...
export const markReservationMessagesRead = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 reservation_id: z.string().uuid(),
 ...
 let q = supabaseAdmin.from("reservation_messages").update(patch).eq("reservation_id"data.reservation_id).eq(readCol, false);
...
export const markReservationReadByChauffeur = createServerFn({ method:"POST" }).inputValidator((input) => z.object({ reservation_id: z.string().uuid() }).parse(input)).handler(async ({ data }) => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { data: count, error } = await supabaseAdmin.rpc("mark_reservation_read_by_chauffeur"{ p_reservation_id: data.reservation_id },
 );
 if (error) throw new Error(error.message);
 return { updated: (count as number)?? 0 };
 });

export const countUnreadForClient = createServerFn({ method:"POST" }).inputValidator((input) => z.object({ reservation_ids: z.array(z.string().uuid()).max(200) }).parse(input)).handler(async ({ data }) => {
 if (data.reservation_ids.length === 0) return {} as Record<string, number>;
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { data: rows, error } = await supabaseAdmin.from("reservation_messages").select("reservation_id").in("reservation_id"data.reservation_ids).eq("sender""chauffeur").eq("read_by_client"false);
 if (error) throw new Error(error.message);
...
export const listAdminChatThreads = createServerFn({ method:"GET" }).handler(async () => {
 ...
 const { data: msgs, error } = await supabaseAdmin.from("reservation_messages").select("reservation_id, sender, content, read_by_chauffeur, created_at").order("created_at"{ ascending: false }).limit(2000);
 if (error) throw new Error(error.message);
 ...
 const { data: resas } = await supabaseAdmin.from("reservations").select("id, client_name, nom, client_phone, telephone, depart, destination, arrivee, status").in("id"ids);
...
export const listSuiviMessages = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 suivi_key: z.string().trim().min(6).max(200),
 before: z.string().datetime().optional(),
 limit: z.number().int().min(1).max(100).optional(),
 ...
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 let q = supabaseAdmin.from("reservation_messages").select("id,reservation_id,sender,content,read_by_client,read_by_chauffeur,created_at").eq("reservation_id"r.id).order("created_at"{ ascending: false }).limit(data.limit?? 60);
 if (data.before) q = q.lt("created_at"data.before);
 const { data: rows, error } = await q;
 if (error) throw new Error(error.message);
 return ((rows?? []) as ChatMessage[]).slice().reverse();
...
export const sendSuiviClientMessage = createServerFn({ method:"POST" }).inputValidator((input) => suiviSendSchema.parse(input)).handler(async ({ data }) => {
 const r = await resolveSuiviReservation(data.suivi_key);
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const clientName = (r as any).client_name || (r as any).nom ||"Client";
 ...
 title: `💬 Message de ${clientName}`,
 body: data.content.slice(0, 100),
 url:"/driver"tag: `chat-driver-resa-${r.id}-${Date.now()}`,
 requireInteraction: false,
 });
 } catch (e) {
...
export const seedReservationSpecialRequest = createServerFn({ method:"POST" }).inputValidator((input) => seedSpecialSchema.parse(input)).handler(async ({ data }) => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 ...
 const { data: existing } = await supabaseAdmin.from("reservation_messages").select("id").eq("reservation_id"data.reservation_id).eq("sender""client").eq("content"data.content).limit(1).maybeSingle();
...
export const sendDirectClientMessage = createServerFn({ method:"POST" }).inputValidator((input) => directSendSchema.extend({ role: z.literal("client") }).parse(input)).handler(async ({ data }) => {
 const accountId = await resolveDirectAccount(data);
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

 // Récupère nom client pour le titre push
 const { data: acct } = await supabaseAdmin.from("client_accounts").select("client_name, email").eq("id"accountId).maybeSingle();
 const clientName = (acct as any)?.client_name || (acct as any)?.email ||"Client";

 const { data: row, error } = await supabaseAdmin.from("direct_messages").insert({
 ...
 title: `💬 Message de ${clientName}`,
 body: data.content.slice(0, 100),
 url:"/driver"tag: `chat-driver-direct-${accountId}-${Date.now()}`,
 requireInteraction: false,
 });
 } catch (e) {
...
export const sendDirectChauffeurMessage = createServerFn({ method:"POST" }).inputValidator((input) => directSendSchema.extend({ role: z.literal("chauffeur") }).parse(input)).handler(async ({ data }) => {
 const accountId = await resolveDirectAccount(data);
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { data: row, error } = await supabaseAdmin.from("direct_messages").insert({
 client_account_id: accountId,
 ...
 await sendPushToAudience("client"{
 title:"💬 Patricia a répondu à votre message"body: data.content.slice(0, 100),
 url:"/client/chat"tag: `chat-client-direct-${accountId}`,
 requireInteraction: false,
 },
 { accountId },
 );
...
export const listDirectMessages = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 before: z.string().datetime().optional(),
 limit: z.number().int().min(1).max(100).optional(),
 }).merge(directAuthSchema).parse(input),
 ).handler(async ({ data }) => {
 const accountId = await resolveDirectAccount(data);
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 let q = supabaseAdmin.from("direct_messages").select("id,client_account_id,sender,content,read_by_client,read_by_chauffeur,created_at").eq("client_account_id"accountId).order("created_at"{ ascending: false }).limit(data.limit?? 30);
 if (data.before) q = q.lt("created_at"data.before);
 const { data: rows, error } = await q;
 if (error) throw new Error(error.message);
...
export const markDirectMessagesRead = createServerFn({ method:"POST" }).inputValidator((input) =>
 ...
 const patch = data.role ==="client"? { read_by_client: true }: { read_by_chauffeur: true };
 const readCol = data.role ==="client"?"read_by_client":"read_by_chauffeur";
 const { error } = await supabaseAdmin.from("direct_messages").update(patch).eq("client_account_id"accountId).eq("sender"peer).eq(readCol, false);
 if (error) throw new Error(error.message);
 return { ok: true };
 });
...
export const listAdminDirectThreads = createServerFn({ method:"GET" }).handler(async () => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { data: msgs, error } = await supabaseAdmin.from("direct_messages").select("client_account_id,sender,content,read_by_chauffeur,created_at").order("created_at"{ ascending: false }).limit(2000);
 if (error) throw new Error(error.message);
...
export const countUnreadChauffeurMessages = createServerFn({ method:"GET" }).handler(async () => {
 ...
 supabaseAdmin.from("direct_messages").select("id"{ count:"exact"head: true }).eq("sender""client").eq("read_by_chauffeur"false),
 supabaseAdmin.from("reservation_messages").select("id"{ count:"exact"head: true }).eq("sender""client").eq("read_by_chauffeur"false),
 ]);
...
export const countUnreadChauffeurForReservation = createServerFn({ method:"POST" }).inputValidator((input) => z.object({ reservation_id: z.string().uuid() }).parse(input)).handler(async ({ data }) => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { count, error } = await supabaseAdmin.from("reservation_messages").select("id"{ count:"exact"head: true }).eq("reservation_id"data.reservation_id).eq("sender""client").eq("read_by_chauffeur"false);
...
export const countUnreadClientForReservation = createServerFn({ method:"POST" }).inputValidator((input) =>
 ...
 const { count, error } = await supabaseAdmin.from("reservation_messages").select("id"{ count:"exact"head: true }).eq("reservation_id"r.id).eq("read_by_client"false);
 if (error) throw error;
 return count?? 0;
 });
...
export const listReservationsWithUnreadChauffeur = createServerFn({ method:"GET" }).handler(
 async () => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { data, error } = await supabaseAdmin.from("reservation_messages").select("reservation_id").eq("sender""client").eq("read_by_chauffeur"false);
 if (error) throw error;
...
export const countUnreadClientForReservationById = createServerFn({ method:"POST" }).inputValidator((input) => z.object({ reservation_id: z.string().uuid() }).parse(input)).handler(async ({ data }) => {
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { count, error } = await supabaseAdmin.from("reservation_messages").select("id"{ count:"exact"head: true }).eq("reservation_id"data.reservation_id).eq("sender""chauffeur").eq("read_by_client"false);
 if (error) throw error;
 return count?? 0;
 });
...
export const getUnreadCountsForReservations = createServerFn({ method:"POST" }).inputValidator((input) =>
 ...
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 const { data: rows, error } = await supabaseAdmin.from("reservation_messages").select("reservation_id,sender,read_by_chauffeur,read_by_client").in("reservation_id"data.reservation_ids);
 if (error) throw error;
 const out: UnreadMap = {};
...
export const listMergedChauffeurThreads = createServerFn({ method:"GET" }).handler(async () => {
 ...
 // 1) Direct messages
 const { data: directs } = await supabaseAdmin.from("direct_messages").select("client_account_id,sender,content,read_by_chauffeur,created_at").order("created_at"{ ascending: false }).limit(2000);

 // 2) Reservation messages
 const { data: resaMsgs } = await supabaseAdmin.from("reservation_messages").select("reservation_id,sender,content,read_by_chauffeur,created_at").order("created_at"{ ascending: false }).limit(2000);
 ...
 if (resaIds.length > 0) {
 const { data: resas } = await supabaseAdmin.from("reservations").select("id, client_account_id, client_name, nom, client_phone, telephone, client_email, email, depart, destination, arrivee, status, pickup_datetime").in("id"resaIds);
 ...
 name: a?.client_name?? a?.email??"Client"phone_display: a?.phone?? null,
 };
 }
 ...
 name: a?.client_name?? r.client_name?? r.nom??"Client"phone_display: a?.phone?? r.client_phone?? r.telephone?? null,
 };
 }
 ...
 email: null,
 phone_tail: ph,
 name: r.client_name?? r.nom??"Client"phone_display: r.client_phone?? r.telephone?? null,
 };
 }
 ...
 email: null,
 phone_tail: null,
 name: r.client_name?? r.nom??"Client"phone_display: null,
 };
 }
 ...
 last_at:""last_content:""last_source:"direct"unread: 0,
 };
 buckets.set(key, b);
 } else {
...
export const loadMergedConversation = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 ...
 const { data: rows } = await supabaseAdmin.from("direct_messages").select("id,sender,content,read_by_chauffeur,created_at").eq("client_account_id"data.client_account_id).order("created_at"{ ascending: false }).limit(lim);
 for (const r of (rows?? []) as any[]) {
 out.push({
 id: r.id,
 source:"direct"reservation_id: null,
 ...
 if (rids.length > 0) {
 const { data: rows } = await supabaseAdmin.from("reservation_messages").select("id,reservation_id,sender,content,read_by_chauffeur,created_at").in("reservation_id"rids).order("created_at"{ ascending: false }).limit(lim);
 const labels = new Map<string, string>();
 for (const id of rids) labels.set(id, `#${String(id).slice(0, 6).toUpperCase()}`);
 for (const r of (rows?? []) as any[]) {
 out.push({
 id: r.id,
 source:"reservation"reservation_id: r.reservation_id,
...
export const markMergedConversationRead = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 client_account_id: z.string().uuid().nullable().optional(),
 reservation_ids: z.array(z.string().uuid()).max(50).optional(),
 }).parse(input),
 ...
 const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
 if (data.client_account_id) {
 await supabaseAdmin.from("direct_messages").update({ read_by_chauffeur: true }).eq("client_account_id"data.client_account_id).eq("sender""client").eq("read_by_chauffeur"false);
 }
 const rids = data.reservation_ids?? [];
 if (rids.length > 0) {
 await supabaseAdmin.from("reservation_messages").update({ read_by_chauffeur: true }).in("reservation_id"rids).eq("sender""client").eq("read_by_chauffeur"false);
 }
 return { ok: true };
 });
...
export const deleteMergedThread = createServerFn({ method:"POST" }).inputValidator((input) =>
 z.object({
 client_account_id: z.string().uuid().nullable().optional(),
 ...
 let deleted = 0;
 if (data.client_account_id) {
 const { count } = await supabaseAdmin.from("direct_messages").delete({ count:"exact" }).eq("client_account_id"data.client_account_id);
 deleted += count?? 0;
 }
 const rids = data.reservation_ids?? [];
 if (rids.length > 0) {
 const { count } = await supabaseAdmin.from("reservation_messages").delete({ count:"exact" }).in("reservation_id"rids);
 deleted += count?? 0;
 }
